...
/**
   ...
   *该库有单独的函数用于操作8位整数、16位整数、1位32位整数和32位浮点值
   * Using the Library
   * ------------
   *
   ...
   * Simply include this file and link the appropriate library in the application and begin calling the library functions. The Library supports 
     single
   * public header file <code> arm_math.h</code> for Cortex-M cores with little endian and big endian. 
   ...
   *
   *
   * The libraries can be built by opening the arm_cortexM_math.uvprojx project in MDK-ARM, selecting a specific target, and defining the optional 
 ...
 *这组函数提供了对正弦、余弦和平方根的快速近似
 * As compared to most of the other functions in the CMSIS math library, the fast math functions
 *与CMSIS数学库中的大多数其他函数相比，此组数学函数更快速
 ...
 * real values.
 *在API函数中，复数数组中的样本数是指复数值的个数;该数组包含两倍于此数量的实值。
 */
 ...
 * below:
 *这组函数提供了基本的矩阵数学运算。这些函数对矩阵数据结构进行操作。例如，类型浮点矩阵结构的定义如下:
 * <pre>
 *     typedef struct
 *     {
 *       uint16_t numRows;     // number of rows of the matrix.     矩阵的行数
 *       uint16_t numCols;     // number of columns of the matrix.  矩阵的列数
 *       float32_t *pData;     // points to the data of the matrix. 指向矩阵的数据
 *     } arm_matrix_instance_f32;
 * </pre>
 * There are similar definitions for Q15 and Q31 data types.
 *对于Q15和Q31数据类型也有类似的定义
 *
 * The structure specifies the size of the matrix and then points to
 * an array of data.  The array is of size <code>numRows X numCols</code>
 * and the values are arranged in row order.  That is, the
 * matrix element (i, j) is stored at:
 *该结构指定矩阵的大小，然后指向一个数据数组。数组的大小为<code>numRows X numCols</code>，值按行顺序排列。即，矩阵元素(i, j)存储在:
 * <pre>
 *     pData[i*numCols + j]
 * </pre>
 *
 ...
 *初始化函数的使用是可选的。但是，如果使用初始化函数那么实例结构就不能放在const数据节中。
 * To place the instance structure in a const data
 * section, manually initialize the data structure.  For example:
 *将实例结构放在const数据中节中，手动初始化数据结构。例如:
 * <pre>
 ...
 * data array.
 *其中<code>nRows</code>指定行数，<code>nColumns</code>指定列数，<code>pData</code>指向数据数组。
 *
 * \par Size Checking
 *尺寸检查
 ...
 *默认情况下，所有矩阵函数都对输入和输出矩阵执行大小检查。
 *例如，矩阵加法函数验证两个输入矩阵和输出矩阵都具有相同的行数和列数。如果大小检查失败，函数返回:
 * <pre>
 ...
 *在库项目设置中。默认情况下，定义此宏并启用大小检查。
 *通过更改项目设置和取消定义这个宏的大小检查被消除，函数运行得更快一点。随着大小检查被禁用，函数总是返回 
 */
...
#define __CMSIS_GENERIC         /* disable NVIC and Systick functions 禁用NVIC和Systick功能*/
...
#undef  __CMSIS_GENERIC         /* enable NVIC and Systick functions 启用NVIC和Systick功能*/
#include "string.h"
#include "math.h"
#ifdef   __cplusplus
extern "C"
...
#define INPUT_SPACING         0xB60B61
  ...
  typedef enum
  {
    ARM_MATH_SUCCESS = 0,                /**< No error                                               没有错误的              */
    ARM_MATH_ARGUMENT_ERROR = -1,        /**< One or more arguments are incorrect                    一个或多个参数不正确    */
    ARM_MATH_LENGTH_ERROR = -2,          /**< Length of data buffer is incorrect                     数据缓冲区长度不正确    */
    ARM_MATH_SIZE_MISMATCH = -3,         /**< Size of matrices is not compatible with the operation. 矩阵的大小与操作不兼容  */
    ARM_MATH_NANINF = -4,                /**< Not-a-number (NaN) or infinity is generated            生成非数字(NaN)或无穷大 */
    ARM_MATH_SINGULAR = -5,              /**< Generated by matrix inversion if the input matrix is singular and cannot be inverted.
	                                                                                                 如果输入矩阵是奇异且不能反转， 
																								     则由矩阵反转生成         */
    ARM_MATH_TEST_FAILURE = -6           /**< Test Failed                                            测试失败                 */
  } arm_status;
...
#define __PACKq7(v0,v1,v2,v3) ( (((int32_t)(v3) <<  0) & (int32_t)0x000000FF) | \
  ...
  q31_t * pRecipTable)
  {
    ...
    /* Convert input sample to 1.31 format 将输入样本转换为1.31格式*/
    in = (in << signBits);

    /* calculation of index for initial approximated Val 初始近似增值税指数的计算*/
    ...
    /* calculation of reciprocal value 倒数值的计算*/
    /* running approximation for two iterations两个迭代的运行近似 */
    for (i = 0U; i < 2U; i++)
    {
...
/*  return (sum + ((q15_t) (x >> 16) * (q15_t) y)) + ((q15_t) x * (q15_t) (y >> 16)); */
  ...
  typedef struct
  {
    uint16_t numTaps;        /**< number of filter coefficients in the filter.                        过滤器中过滤系数的个数           */
    q7_t *pState;            /**< points to the state variable array. The array is of length numTaps+blockSize-1.
	                                                                                                  指向状态变量数组。
																									  数组的长度为numTapstblockSize-1  */
    q7_t *pCoeffs;           /**< points to the coefficient array. The array is of length numTaps.    指向系数数组。数组的长度是numTaps*/
  } arm_fir_instance_q7;
   ...
   *用于Cortex-M3和Cortex-M4的快速Q15 FIR滤波器处理功能
   ...
   *如果初始化成功，函数返回ARM_MATH_SUCCESS;如果<code>numTaps</code>不支持值，函数返回ARM_MATH_ARGUMENT_ERROR
   */
   ...
   *用于Cortex-M3和Cortex-M4的快速Q31 FIR滤波器处理功能
   ...
   *Q15 Biquad级联滤波器的实例结构
   */
  typedef struct
  {
    int8_t numStages;        /**< number of 2nd order stages in the filter.  Overall order is 2*numStages. 
	                              滤波器的二阶级数                           总的顺序是2*numStages                */
    q15_t *pState;           /**< Points to the array of state coefficients.  The array is of length 4*numStages.
	                             指向状态系数数组                              数组的长度是4*numStages            */
    q15_t *pCoeffs;          /**< Points to the array of coefficients.  The array is of length 5*numStages. 
	                              指向系数数组                          数组的长度是5*numStages                   */
    int8_t postShift;        /**< Additional shift, in bits, applied to each output sample. 
	                              附加移位，以位为单位，应用于每个输出样本                                        */
   ...
   *用于Cortex-M3和Cortex-M4l的Q15 Biquad级联滤波器的快速但不太精确的处理功能
   * @param[in]  S          points to an instance of the Q15 Biquad cascade structure.
   * @param[in]  pSrc       points to the block of input data.
   * @param[out] pDst       points to the block of output data.
   * @param[in]  blockSize  number of samples to process.
   ...
   *用于Cortex-M3和Cortex-M4的Q31双级联滤波器的快速但不太精确的处理功能
   * @param[in]  S          points to an instance of the Q31 Biquad cascade structure.
   * @param[in]  pSrc       points to the block of input data.
   * @param[out] pDst       points to the block of output data.
   * @param[in]  blockSize  number of samples to process.
   ...
   *Cortex-M3和Cortex-M4的Q15矩阵乘法(快速变体)
   * @param[in]  pSrcA   points to the first input matrix structure
   * @param[in]  pSrcB   points to the second input matrix structure
   * @param[out] pDst    points to output matrix structure
   * @param[in]  pState  points to the array for storing intermediate results
   ...
   *用于Cortex-M3和Cortex-M4的Q31矩阵乘法(快速变体)
   * @param[in]  pSrcA  points to the first input matrix structure
   * @param[in]  pSrcB  points to the second input matrix structure
   * @param[out] pDst   points to output matrix structure
   * @return     The function returns either
  ...
  typedef struct
  {
    uint16_t numRows;   /**< number of rows in the data table.   数据表中的行数 */
    uint16_t numCols;   /**< number of columns in the data table.数据表中的列数 */
    float32_t *pData;   /**< points to the data table.           指向数据表     */
  ...
  typedef struct
  {
    ...
    uint8_t ifftFlag;                /**< flag that selects forward (ifftFlag=0) or inverse (ifftFlag=1) transform. 
	                                      选择正向(ifftFlag=0)或反向(ifftFlag-1)转换的标志                             */
    uint8_t bitReverseFlag;          /**< flag that enables (bitReverseFlag=1) or disables (bitReverseFlag=0) bit reversal of output. 
	                                      启用(bitReverseflag=1)或禁用(bitReverseflag=0)输出位反转的标志               */
    q15_t *pTwiddle;                 /**< points to the Sin twiddle factor table. 
	                                      指向sintwiddle因子表                                                         */
    uint16_t *pBitRevTable;          /**< points to the bit reversal table. 
	                                      指向位反转表                                                                 */
    uint16_t twidCoefModifier;       /**< twiddle coefficient modifier that supports different size FFTs with the same twiddle factor table. 
	                                      旋转系数修改器，支持具有相同旋转因子表的不同大小的fft                        */
    uint16_t bitRevFactor;           /**< bit reversal modifier that supports different size FFTs with the same bit reversal table. 
	                                      位反转修饰符，支持不同大小的fft与相同的位反转表                              */
...
void arm_cfft_q31(
  ...
  typedef struct
  {
    uint16_t fftLen;                   /**< length of the FFT.                     FFT的长度        */
    const float32_t *pTwiddle;         /**< points to the Twiddle factor table.   指向Twiddle因子表 */
    const uint16_t *pBitRevTable;      /**< points to the bit reversal table.     指向位反转表。    */
    uint16_t bitRevLength;             /**< bit reversal table length.            位反转表长度      */
