...
*                                            CREATE A TASK
*
* Description: :用于uC/OS-II管理任务的执行。任务既可以在多任务开始之前创建，也可以由正在运行的任务创建。ISR不能创建任务。
*
* Arguments  : task    是一个指针指向任务代码
*
*              p_arg    是一个指向可选数据区的指针，可用于在任务第一次执行时向任务传递参数。任务认为它被调用并传递了参数'p_arg'，
*                       如下所示:
*
...
*                           }
*
*              ptos     是指向任务堆栈顶部的指针。如果配置常数OS_STK_GROWTH设置为1，则假定堆栈向下增长(即从高内存到低内存)。
*                       因此，'pstk'将指向堆栈的最高(有效)内存位置。如果OS_STK_GROWTH设置为0，'pstk'将指向堆栈的最低内存位置，
*                       堆栈将随着内存位置的增加而增长。
*
*              prio     是任务的优先级。必须为每个任务分配一个唯一的优先级，数字越低，优先级越高。
*
* Returns    : OS_ERR_NONE             如果函数成功。
*              OS_PRIO_EXIT            如果任务优先级已经存在(每个任务必须有唯一的优先级)。
*              OS_ERR_PRIO_INVALID     如果指定的优先级高于允许的最大优先级(即>= OS_LOWEST_PRIO)
*              OS_ERR_TASK_CREATE_ISR  如果您尝试从ISR(中断服务程序)创建任务。
*********************************************************************************************************
*/
...
*                                            SUSPEND A TASK
*
* Description: 调用这个函数来挂起一个任务。如果传递给OSTaskSuspend()的优先级与调用任务或oS_PRIO_SELF的优先级相同，
*              则该任务可以是调用任务。
*
