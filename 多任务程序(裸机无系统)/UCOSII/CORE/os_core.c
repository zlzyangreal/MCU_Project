...
*
* Returns    : none
* 感觉这就是系统初始化，初始化系统的所有配置
*********************************************************************************************************
*/

void  OSInit (void)
{
    OSInitHookBegin();                                           /* 呼叫端口特定初始化代码(我的理解是刷新系统从0开始计数)   */
    ...
    OS_InitEventList();                                          /* 初始化OS_EVENTS的空闲列表    */
...
*                                          START MULTITASKING
*
* Description: 此函数用于启动多任务进程，该进程允许uc/os-II管理您创建的任务。在你可以调用osstart()之前，
*              你必须调用OSInit()并且你必须创建至少一个任务。
*
* Arguments  : none
*
* Returns    : none
